...
export function useMasterCompanyEditPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const response = await perusahaanApi.update(id.value, payload as any);

      if (response.success) {
        toast.add({ title: "Perusahaan berhasil diperbarui", color: "success" });
        cleanupMap();
        router.push("/admin/master/companies");
      } else {
        toast.add({
