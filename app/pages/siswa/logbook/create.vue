<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center gap-3">
      <UButton variant="ghost" color="neutral" size="sm" @click="navigateTo('/siswa/logbook')">
        <Icon name="lucide:arrow-left" class="w-5 h-5" />
      </UButton>
      <div>
        <h1 class="text-xl font-bold text-slate-900">Tulis Logbook</h1>
        <p class="text-sm text-slate-500">Catat kegiatan PKL harianmu</p>
      </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-5 lg:p-6">
      <form @submit.prevent="submitForm" class="space-y-5">
        ...
        <UFormField label="Foto Kegiatan (Opsional)">
          ...
          >
            ...
            <div v-if="form.fotoPreview">
              <img :src="form.fotoPreview" class="w-32 h-32 mx-auto rounded-xl object-cover mb-3" />
              <p class="text-sm text-slate-600">Klik untuk ganti foto</p>
              <UButton type="button" variant="ghost" color="error" size="xs" class="mt-2" @click.stop="removePhoto">
            ...
            <div v-else>
              <div class="w-16 h-16 rounded-xl bg-slate-100 flex items-center justify-center mx-auto mb-3">
                <Icon name="lucide:image-plus" class="w-8 h-8 text-slate-400" />
              </div>
              <p class="text-sm text-slate-600">Drag & drop atau klik untuk upload</p>
        ...
        <div class="flex gap-3 pt-4">
          <UButton type="button" variant="outline" color="neutral" class="flex-1" @click="navigateTo('/siswa/logbook')">
            Batal
          </UButton>
