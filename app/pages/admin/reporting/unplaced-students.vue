<template>
    <div class="space-y-6">
        ...
        <!-- Hero Header -->
        <div
            class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-rose-500 via-red-600 to-orange-700 p-6 text-white shadow-xl">
            ...
            <div class="relative flex items-center justify-between flex-wrap gap-4">
                <div class="flex items-center gap-4">
                    <div
                        ...
                        <Icon name="lucide:user-x" class="w-7 h-7 text-white" />
                    </div>
                    <div>
                        <h1 class="text-2xl font-bold tracking-tight">Laporan Siswa Belum Mendapat Tempat</h1>
                        <p class="text-rose-200 text-sm mt-0.5">Early warning system – siswa yang belum ditempatkan
                            magang
                        </p>
                    </div>
                </div>
        ...
        <!-- Statistics Cards -->
        <div v-if="statistics" class="grid grid-cols-2 lg:grid-cols-3 gap-4">
            <div class="bg-white rounded-xl border border-slate-200 p-5 hover:shadow-md transition-all hover:-translate-y-0.5"
                :class="{ 'border-red-200 bg-red-50/50': (statistics.total_unplaced ?? 0) > 0 }">
                <div class="flex items-center justify-between mb-3">
                    <div class="w-10 h-10 rounded-xl flex items-center justify-center"
                        :class="(statistics.total_unplaced ?? 0) > 0 ? 'bg-red-100' : 'bg-slate-50'">
                        <Icon name="lucide:users" class="w-5 h-5"
                            :class="(statistics.total_unplaced ?? 0) > 0 ? 'text-red-600' : 'text-slate-500'" />
                    </div>
                    <span v-if="(statistics.total_unplaced ?? 0) > 0"
                        class="text-xs font-medium text-red-600 bg-red-100 px-2 py-1 rounded-full animate-pulse">
                        Perlu Tindakan!
                    </span>
                    <span v-else
                        class="text-xs font-medium text-slate-400 bg-slate-50 px-2 py-1 rounded-full">Total</span>
                </div>
                <p class="text-3xl font-extrabold"
                    :class="(statistics.total_unplaced ?? 0) > 0 ? 'text-red-700' : 'text-slate-900'">
                    {{ statistics.total_unplaced ?? 0 }}
                </p>
                <p class="text-sm text-slate-500 mt-1">Siswa Belum Ditempatkan</p>
            </div>

            <div
                class="bg-white rounded-xl border border-slate-200 p-5 hover:shadow-md transition-all hover:-translate-y-0.5">
                <div class="flex items-center gap-2 mb-3">
                    <Icon name="lucide:graduation-cap" class="w-5 h-5 text-indigo-600" />
                    <h3 class="text-sm font-bold text-slate-800">Per Jurusan</h3>
                </div>
                <div class="space-y-2 mt-2 max-h-32 overflow-y-auto">
                    <div v-for="stat in statistics.unplaced_by_major" :key="stat.major"
                        class="flex justify-between items-center text-sm">
                        <span class="text-slate-600 truncate text-xs">{{ stat.major }}</span>
                        <span
                            class="font-bold text-slate-900 bg-red-100 text-red-700 px-2 rounded-full text-xs ml-2 shrink-0">{{
                            stat.count }}</span>
                    </div>
                    <div v-if="!statistics.unplaced_by_major?.length" class="text-xs text-slate-400">Tidak ada data
                    </div>
                </div>
            </div>
            ...
            <div
                class="bg-white rounded-xl border border-slate-200 p-5 hover:shadow-md transition-all hover:-translate-y-0.5">
                <div class="flex items-center gap-2 mb-3">
                    <Icon name="lucide:book-open" class="w-5 h-5 text-amber-600" />
                    <h3 class="text-sm font-bold text-slate-800">Per Kelas</h3>
                </div>
                <div class="space-y-2 mt-2 max-h-32 overflow-y-auto">
                    <div v-for="stat in statistics.unplaced_by_class" :key="stat.className"
                        class="flex justify-between items-center text-sm">
                        <span class="text-slate-600 truncate text-xs">{{ stat.className }}</span>
                        <span
                            class="font-bold text-slate-900 bg-amber-100 text-amber-700 px-2 rounded-full text-xs ml-2 shrink-0">{{
                            stat.count }}</span>
                    </div>
                    <div v-if="!statistics.unplaced_by_class?.length" class="text-xs text-slate-400">Tidak ada data
                    </div>
                </div>
            </div>
        </div>
            ...
            <button @click="isFilterOpen = !isFilterOpen"
                ...
                <div class="flex items-center gap-3">
                    <div class="w-8 h-8 rounded-lg bg-rose-50 flex items-center justify-center">
                        <Icon name="lucide:sliders-horizontal" class="w-4 h-4 text-rose-600" />
                    </div>
                    <span class="font-semibold text-slate-800 text-sm">Filter & Pencarian</span>
                    <span v-if="activeFilterCount > 0"
                        class="text-xs font-bold text-rose-700 bg-rose-100 px-2 py-0.5 rounded-full">
                        ...
                        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            <div>
                                ...
                                <input v-model="filterDraft.jurusan" type="text" placeholder="Contoh: RPL, TKJ"
                                    class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-rose-500 focus:border-rose-500 transition-shadow bg-slate-50 focus:bg-white" />
                            </div>
                            <div>
                                <label
                                    class="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2 block">Kelas</label>
                                <input v-model="filterDraft.kelas" type="text" placeholder="Contoh: XII RPL 1"
                                    class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-rose-500 focus:border-rose-500 transition-shadow bg-slate-50 focus:bg-white" />
                            </div>
                        </div>
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between gap-4 flex-wrap">
                <div>
                    <h2 class="font-semibold text-slate-900">Daftar Siswa Belum Mendapat Tempat</h2>
                    <p class="text-xs text-slate-400 mt-0.5">{{ filteredData.length }} siswa ditemukan</p>
                </div>
                <div class="flex items-center gap-2">
                    <div class="relative">
                        <input v-model="searchQuery" placeholder="Cari nama / kelas..."
                            class="pl-9 pr-3 py-1.5 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-rose-500 focus:border-rose-500 bg-slate-50 focus:bg-white w-56 transition-all" />
            ...
            <div v-else class="overflow-x-auto">
                    ...
                    <tbody class="divide-y divide-slate-50">
                        <tr v-for="(row, i) in filteredData" :key="i"
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="w-9 h-9 rounded-xl bg-gradient-to-br from-rose-400 to-red-600 flex items-center justify-center text-white font-bold text-sm shrink-0 shadow-sm">
                                        {{ (row.nama_siswa || '?').charAt(0).toUpperCase() }}
                                    </div>
                                    <div>
                                        <p class="font-semibold text-slate-800">{{ row.nama_siswa }}</p>
                            ...
                            <td class="px-5 py-4">
                                <span
                                    ...
                                    {{ row.kelas?.jurusan?.nama_jurusan || '-' }}
                                </span>
                            </td>
                            <td class="px-5 py-4 text-center">
                                <span class="font-bold text-slate-800">{{ row.total_pengajuan ?? 0 }}</span>
                                <span class="text-xs text-slate-400 ml-1">diproses</span>
                            </td>
                            <td class="px-5 py-4 text-center">
                                <template v-if="!row.status_terakhir">
                                    <span
                                        class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-slate-100 text-slate-600">
                                        <span class="w-1.5 h-1.5 rounded-full bg-slate-400"></span>Belum Mengajukan
                                    </span>
                                </template>
                                <template v-else-if="row.status_terakhir === 'ditolak'">
                                    <span
                                        class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-red-100 text-red-700">
                                        <span class="w-1.5 h-1.5 rounded-full bg-red-500"></span>Ditolak
                                    </span>
                                </template>
                                <template v-else-if="row.status_terakhir === 'menunggu'">
                                    <span
                ...
                <div v-if="filteredData.length === 0"
                    ...
                    <Icon name="lucide:check-circle" class="w-12 h-12 opacity-30 text-emerald-500" />
                    <p class="font-medium">Semua siswa sudah mendapatkan tempat!</p>
                    <p class="text-xs">Atau coba ubah filter pencarian</p>
                </div>
            </div>
                ...
                <div class="p-6">
                    <div class="flex items-center gap-3 mb-4">
                        <div class="w-10 h-10 rounded-xl bg-rose-100 flex items-center justify-center">
                            <Icon name="lucide:download" class="w-5 h-5 text-rose-600" />
                        </div>
                        <div>
                    ...
                    <div v-if="exporting" class="mt-4 flex items-center justify-center gap-2 text-sm text-slate-600">
                        <div class="w-4 h-4 border-2 border-rose-200 border-t-rose-600 rounded-full animate-spin">
                        </div>
    ...
    if (q) {
        data = data.filter(
            (r) =>
                ...
                r.kelas?.nama_kelas?.toLowerCase().includes(q) ||
                r.nis?.toLowerCase().includes(q)
        );
    }
    if (filterDraft.value.jurusan) {
        data = data.filter((r) =>
            r.kelas?.jurusan?.nama_jurusan?.toLowerCase().includes(filterDraft.value.jurusan.toLowerCase())
        );
    }
    if (filterDraft.value.kelas) {
        data = data.filter((r) =>
            r.kelas?.nama_kelas?.toLowerCase().includes(filterDraft.value.kelas.toLowerCase())
        );
    }
    return data;
});
...
async function fetchData() {
    loading.value = true;
    try {
        const response = await api.getUnplacedStudentsReport({});
        if (response.success) {
            reportData.value = response.data || [];
            statistics.value = response.statistics || null;
        }
        lastUpdated.value = new Date().toLocaleTimeString("id-ID");
    } catch (error) { console.error("Error fetching unplaced students report:", error); } finally { loading.value = false; }
}

function applyFilters() { fetchData(); }
function resetFilters() {
    filterDraft.value = { jurusan: "", kelas: "" };
    searchQuery.value = "";
    fetchData();
}
...
async function handleExport(format: "pdf" | "excel") {
    ...
    try {
        ...
        const blob = await api.exportReport("unplaced-students", params);
        const url = window.URL.createObjectURL(blob);
        const filename = `laporan-siswa-belum-tempat-${new Date().getTime()}.${format === "pdf" ? "pdf" : "xlsx"}`;
...
function downloadPreviewedPdf() {
    ...
    link.download = previewFilename.value || `laporan-siswa-belum-tempat-${new Date().getTime()}.pdf`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
