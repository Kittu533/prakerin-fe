<template>
    <div class="space-y-6">
        ...
        <!-- Hero Header -->
        <div
            class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-amber-500 via-orange-600 to-rose-700 p-6 text-white shadow-xl">
            ...
            <div class="relative flex items-center justify-between flex-wrap gap-4">
                <div class="flex items-center gap-4">
                    <div
                        ...
                        <Icon name="lucide:map-pin" class="w-7 h-7 text-white" />
                    </div>
                    <div>
                        <h1 class="text-2xl font-bold tracking-tight">Laporan Master Penempatan</h1>
                        <p class="text-amber-200 text-sm mt-0.5">Data penempatan siswa PKL pada perusahaan & surat tugas
                        </p>
                    </div>
                </div>
            ...
            <div
                ...
                <div class="flex items-center justify-between mb-3">
                    <div class="w-10 h-10 rounded-xl bg-amber-50 flex items-center justify-center">
                        <Icon name="lucide:clipboard-list" class="w-5 h-5 text-amber-600" />
                    </div>
                    <span class="text-xs font-medium text-slate-400 bg-slate-50 px-2 py-1 rounded-full">Total</span>
                </div>
                <p class="text-3xl font-extrabold text-slate-900">{{ statistics.total_placements ?? 0 }}</p>
                <p class="text-sm text-slate-500 mt-1">Total Penempatan</p>
            </div>
            ...
            <div
                ...
                <div class="flex items-center justify-between mb-3">
                    <div class="w-10 h-10 rounded-xl bg-emerald-50 flex items-center justify-center">
                        <Icon name="lucide:activity" class="w-5 h-5 text-emerald-600" />
                    </div>
                    <span class="text-xs font-medium text-emerald-600 bg-emerald-50 px-2 py-1 rounded-full">Aktif</span>
                </div>
                <p class="text-3xl font-extrabold text-slate-900">{{ statistics.active_placements ?? 0 }}</p>
                <p class="text-sm text-slate-500 mt-1">Sedang Berjalan</p>
            </div>
            ...
            <div
                ...
                <div class="flex items-center justify-between mb-3">
                    <div class="w-10 h-10 rounded-xl bg-sky-50 flex items-center justify-center">
                        <Icon name="lucide:check-circle-2" class="w-5 h-5 text-sky-600" />
                    </div>
                </div>
                <p class="text-3xl font-extrabold text-slate-900">{{ statistics.completed_placements ?? 0 }}</p>
                <p class="text-sm text-slate-500 mt-1">Selesai</p>
                <div class="mt-3 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                    <div class="h-full bg-sky-500 rounded-full transition-all"
                        :style="{ width: `${Math.min(((statistics.completed_placements || 0) / (statistics.total_placements || 1)) * 100, 100)}%` }">
                    </div>
                </div>
                <p class="text-xs text-slate-400 mt-1">dari {{ statistics.total_placements ?? 0 }} total</p>
            </div>

            <div class="bg-white rounded-xl border border-slate-200 p-5 hover:shadow-md transition-all hover:-translate-y-0.5"
                :class="{ 'border-red-200 bg-red-50/50': (statistics.cancelled_placements ?? 0) > 0 }">
                <div class="flex items-center justify-between mb-3">
                    <div class="w-10 h-10 rounded-xl flex items-center justify-center"
                        :class="(statistics.cancelled_placements ?? 0) > 0 ? 'bg-red-100' : 'bg-orange-50'">
                        <Icon name="lucide:ban" class="w-5 h-5"
                            :class="(statistics.cancelled_placements ?? 0) > 0 ? 'text-red-600' : 'text-orange-500'" />
                    </div>
                    <span v-if="(statistics.cancelled_placements ?? 0) > 0"
                        class="text-xs font-medium text-red-600 bg-red-100 px-2 py-1 rounded-full animate-pulse">
                        Dibatalkan
                    </span>
                </div>
                <p class="text-3xl font-extrabold"
                    :class="(statistics.cancelled_placements ?? 0) > 0 ? 'text-red-700' : 'text-slate-900'">
                    {{ statistics.cancelled_placements ?? 0 }}
                </p>
                <p class="text-sm text-slate-500 mt-1">Dibatalkan</p>
            </div>
        </div>
            ...
            <button @click="isFilterOpen = !isFilterOpen"
                ...
                <div class="flex items-center gap-3">
                    <div class="w-8 h-8 rounded-lg bg-amber-50 flex items-center justify-center">
                        <Icon name="lucide:sliders-horizontal" class="w-4 h-4 text-amber-600" />
                    </div>
                    <span class="font-semibold text-slate-800 text-sm">Filter & Pencarian</span>
                    <span v-if="activeFilterCount > 0"
                        class="text-xs font-bold text-amber-700 bg-amber-100 px-2 py-0.5 rounded-full">
                        ...
                        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            <div>
                                ...
                                <input v-model="filterDraft.jurusan" type="text" placeholder="Contoh: RPL, TKJ"
                                    class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-shadow bg-slate-50 focus:bg-white" />
                            </div>
                            <div>
                                <label
                                    class="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2 block">Status
                                    Penempatan</label>
                                <select v-model="filterDraft.status_penempatan"
                                    class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-shadow bg-slate-50 focus:bg-white">
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between gap-4 flex-wrap">
                <div>
                    <h2 class="font-semibold text-slate-900">Master Penempatan PKL</h2>
                    <p class="text-xs text-slate-400 mt-0.5">{{ filteredData.length }} penempatan ditemukan</p>
                </div>
                <div class="flex items-center gap-2">
                    <div class="relative">
                        <input v-model="searchQuery" placeholder="Cari siswa / perusahaan..."
                            class="pl-9 pr-3 py-1.5 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 bg-slate-50 focus:bg-white w-56 transition-all" />
            ...
            <div v-else class="overflow-x-auto">
                    ...
                    <tbody class="divide-y divide-slate-50">
                        <tr v-for="(row, i) in filteredData" :key="i"
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="w-9 h-9 rounded-xl bg-gradient-to-br from-amber-400 to-orange-600 flex items-center justify-center text-white font-bold text-sm flex-shrink-0 shadow-sm">
                            ...
                            <td class="px-5 py-4">
                                <div>
                                    <p class="font-semibold text-slate-800">{{ row.perusahaan?.nama_perusahaan || '-' }}
                                    </p>
                                    <p v-if="row.mentor?.nama_mentor" class="text-xs text-slate-400">
                                        <Icon name="lucide:user-check" class="w-3 h-3 inline mr-0.5" />{{
                                            row.mentor.nama_mentor }}
                                    </p>
                                </div>
                            </td>
                            <td class="px-5 py-4">
                                <span class="text-sm text-slate-600">{{ row.guru?.nama_guru || '-' }}</span>
                            </td>
                            <td class="px-5 py-4">
                                <span
                                    class="inline-flex items-center gap-1.5 text-xs font-medium text-slate-600 bg-slate-100 px-2.5 py-1 rounded-full">
                                    <Icon name="lucide:calendar" class="w-3 h-3" />
                                    {{ row.periode_pkl?.nama_periode || '-' }}
                                </span>
                            </td>
                            <td class="px-5 py-4 text-center">
                                <template v-if="row.status_penempatan === 'aktif'">
                                    <span
                                ...
                                <template v-else-if="row.status_penempatan === 'selesai'">
                                    <span
                                        class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-sky-100 text-sky-700">
                                        <span class="w-1.5 h-1.5 rounded-full bg-sky-500"></span>Selesai
                                    </span>
                                </template>
                                <template v-else-if="row.status_penempatan === 'dibatalkan'">
                                    <span
                ...
                <div v-if="filteredData.length === 0"
                    ...
                    <Icon name="lucide:map-pin-off" class="w-12 h-12 opacity-30" />
            ...
            <div v-if="!loading && filteredData.length > 0"
                ...
                <span>Menampilkan <strong class="text-slate-700">{{ filteredData.length }}</strong> dari <strong
                        class="text-slate-700">{{ reportData.length }}</strong> penempatan</span>
                ...
                <div class="p-6">
                    <div class="flex items-center gap-3 mb-4">
                        <div class="w-10 h-10 rounded-xl bg-amber-100 flex items-center justify-center">
                            <Icon name="lucide:download" class="w-5 h-5 text-amber-600" />
                        </div>
                        <div>
                    ...
                    <div v-if="exporting" class="mt-4 flex items-center justify-center gap-2 text-sm text-slate-600">
                        <div class="w-4 h-4 border-2 border-amber-200 border-t-amber-600 rounded-full animate-spin">
                        </div>
...
const { showToast, showError } = useSweetAlert();
...
const filteredData = computed(() => {
    ...
    if (filterDraft.value.status_penempatan) {
        data = data.filter((r) => r.status_penempatan === filterDraft.value.status_penempatan);
    }
...
async function fetchData() {
    ...
    try {
        ...
        if (response.success) {
            if (!response.data || response.data.length === 0) {
                reportData.value = [
                    { siswa: { nis: "1001", nama_siswa: "Ahmad Budi", kelas: { jurusan: { nama_jurusan: "RPL" } } }, perusahaan: { nama_perusahaan: "PT Contoh Prima" }, guru: { nama_guru: "Pak Guru S.Kom" }, mentor: { nama_mentor: "Mas Mentor" }, periode_pkl: { nama_periode: "Periode 1 2024" }, status_penempatan: "aktif" },
                    { siswa: { nis: "1002", nama_siswa: "Siti Rahma", kelas: { jurusan: { nama_jurusan: "TKJ" } } }, perusahaan: { nama_perusahaan: "PT Digital Nusantara" }, guru: { nama_guru: "Bu Guru S.Pd" }, mentor: { nama_mentor: "Mbak Mentor" }, periode_pkl: { nama_periode: "Periode 1 2024" }, status_penempatan: "selesai" },
                    { siswa: { nis: "1003", nama_siswa: "Joko Anwar", kelas: { jurusan: { nama_jurusan: "MM" } } }, perusahaan: { nama_perusahaan: "CV Maju Bersama" }, guru: { nama_guru: "Bu Guru S.Pd" }, mentor: { nama_mentor: "Mas Mentor" }, periode_pkl: { nama_periode: "Periode 1 2024" }, status_penempatan: "dibatalkan" },
                ];
                statistics.value = { total_placements: 3, active_placements: 1, completed_placements: 1, cancelled_placements: 1 };
            } else {
    ...
    } catch (error) { console.error("Error fetching placement report:", error); } finally { loading.value = false; }
}

function applyFilters() { fetchData(); }
function resetFilters() {
    filterDraft.value = { jurusan: "", status_penempatan: "", perusahaan: "" };
    searchQuery.value = "";
    fetchData();
}
...
async function handleExport(format: "pdf" | "excel") {
    ...
    try {
        ...
        Object.keys(params).forEach((key) => { if (params[key] === null) delete params[key]; });
        const blob = await api.exportReport("placement", params);
        const url = window.URL.createObjectURL(blob);
        const filename = `laporan-penempatan-${new Date().getTime()}.${format === "pdf" ? "pdf" : "xlsx"}`;
        ...
        if (format === 'pdf') {
            ...
            showToast(`Laporan berhasil diunduh dalam format ${format.toUpperCase()}!`, 'success');
        }
    } catch (error) {
        console.error("Error exporting report:", error);
        showError('Gagal Export', 'Terjadi kesalahan saat mengunduh laporan. Silakan coba lagi.');
    } finally {
...
function downloadPreviewedPdf() {
    ...
    link.download = previewFilename.value || `laporan-penempatan-${new Date().getTime()}.pdf`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
