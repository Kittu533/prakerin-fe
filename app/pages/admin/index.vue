        ...
        <div class="bg-sky-500 rounded-2xl p-5 lg:p-6 text-white">
            <div class="flex items-start justify-between gap-4">
                ...
                <UButton
                    ...
                    variant="solid"
                    size="sm"
                    class="shrink-0 bg-white text-sky-600 hover:bg-sky-50"
                >
            ...
            <UTable v-else :data="recentApps" :columns="columns">
                <template #siswa-cell="{ row }">
                    <div class="flex items-center gap-2">
                        <div
                            class="w-8 h-8 rounded-lg bg-sky-100 flex items-center justify-center text-sky-600 text-xs font-semibold"
                        >
                            {{
                                row.original.siswa
                                    .split(" ")
...
import {
    useDashboardApi,
    type DashboardStats,
    type PenempatanPerJurusan,
    type TrendPengajuan,
    type StatusSiswa,
    type RecentPengajuan,
} from "~/composables/api/use-reporting";
...
async function fetchDashboardData() {
    ...
    try {
        ...
        const [statsRes, penempatanRes, trendRes, statusRes, recentRes] =
        ...
        if (
            ...
            if (stats.value[1])
                stats.value[1].value = s.total_pengajuan_pending ?? 0;
            if (stats.value[2])
                stats.value[2].value = s.total_penempatan_aktif ?? 0;
            if (stats.value[3]) stats.value[3].value = 0; // masalahAbsensi not available from backend

            // Update alerts
            if (alerts.value[0])
                alerts.value[0].title = `${s.total_pengajuan_pending ?? 0} pengajuan baru`;
            if (alerts.value[1])
                alerts.value[1].title = `0 siswa absensi rendah`;
        }
        ...
        if (
            ...
            if (data.length > 0) {
                placementChartOptions.xaxis.categories = data.map(
                    (d) => d.jurusan,
                );
                placementChartSeries.value = [
                    { name: "Penempatan", data: data.map((d) => d.persentase) },
                ];
            }
        }
        ...
        if (recentRes.status === "fulfilled" && recentRes.value?.success) {
            ...
            if (Array.isArray(data)) {
                recentApps.value = data as RecentPengajuan[];
            } else if (data?.data && Array.isArray(data.data)) {
                recentApps.value = data.data as RecentPengajuan[];
            } else if (data?.response && Array.isArray(data.response)) {
                recentApps.value = data.response as RecentPengajuan[];
            } else {
                recentApps.value = [];
            }
        }
...
function useDummyData() {
    ...
    alerts.value = [
        ...
        },
        {
            ...
            color: "primary" as const,
            icon: "lucide:info",
            title: "Kuota PT Telkom hampir penuh",
            desc: "Sisa 1 slot",
        },
    ];

    placementChartSeries.value = [
        { name: "Penempatan", data: [85, 72, 68, 55] },
    ];
    trendChartSeries.value = [
