<template>
    <div class="space-y-4">
            ...
            <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-white/80 text-sm">
                        {{
                            isApproved
                                ? "Selamat! Pengajuan PKL Anda telah disetujui. Penempatan PKL telah dibuat secara otomatis."
                                : isRejected
                                  ? "Maaf, pengajuan PKL Anda ditolak. Silakan ajukan kembali dengan perusahaan lain."
        ...
        <!-- Success Info Card (Approved) -->
        <div
            v-if="isApproved"
            class="bg-emerald-50 rounded-xl border border-emerald-200 p-5"
        >
                ...
                <div>
                    ...
                    <ul class="text-sm text-emerald-700 space-y-1">
                        <li class="flex items-start gap-2">
                            ...
                            <span
                                >Lakukan absensi harian melalui menu
                                Absensi</span
                            >
                        </li>
                        <li class="flex items-start gap-2">
                            <span class="text-emerald-500">•</span>
                            <span
                                >Isi logbook kegiatan harian untuk dokumentasi
                                PKL</span
                            >
                        </li>
                        <li class="flex items-start gap-2">
                            <span class="text-emerald-500">•</span>
                            <span
                                >Perhatikan jadwal dan aturan PKL di
                                perusahaan</span
                            >
                        </li>
                        <li class="flex items-start gap-2">
                            <span class="text-emerald-500">•</span>
                            <span
                                >Hubungi guru pembimbing jika ada kendala</span
                            >
                        </li>
                    </ul>
                </div>
        ...
        >
            <div
                class="bg-slate-50 px-5 py-3 border-b border-slate-200 flex items-center justify-between"
            >
            ...
            <div class="p-5 space-y-4">
                <div class="grid sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <p class="text-sm font-medium text-slate-900">
                            {{
                                application?.perusahaan?.nama_perusahaan || "-"
                            }}
                        </p>
                    </div>
                    <div>
                        ...
                        <p class="text-sm text-slate-900">
                            {{ application?.perusahaan?.alamat || "-" }}
                        </p>
                    </div>
                    <div>
                        <p class="text-xs text-slate-500 mb-1">Bidang Usaha</p>
                        <p class="text-sm text-slate-900">
                            {{ application?.perusahaan?.bidang_usaha || "-" }}
                        </p>
                    </div>
                    <div>
                        ...
                        <p class="text-sm text-slate-900">
                            {{ formatDate(application?.tanggal_pengajuan) }}
                        </p>
                    </div>
                    <div>
                        <p class="text-xs text-slate-500 mb-1">
                ...
                <!-- Rejection Reason -->
                <div
                    v-if="isRejected && application?.catatan"
                ...
                <div class="flex items-center gap-2 overflow-x-auto pb-2">
                    <template v-for="(step, i) in timelineSteps" :key="i">
                        <div class="flex items-center gap-2 shrink-0">
                            ...
                            <span
                                class="text-sm font-medium whitespace-nowrap hidden sm:inline"
                                :class="
                        ...
                        <div
                            ...
                            class="w-12 sm:w-20 h-1 shrink-0 transition-all"
                            :class="
...
const isApproved = computed(() => {
    return props.application?.status_pengajuan?.toLowerCase() === "disetujui";
});
...
const formattedStatus = computed(() => {
    ...
    return (
        statusMap[status || ""] || props.application?.status_pengajuan || "-"
    );
});
