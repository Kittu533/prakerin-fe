    ...
    >
        ...
        >
            <div
                ...
                :class="isMinimized ? 'justify-center w-full' : ''"
            >
                <img
        ...
        <nav class="flex-1 overflow-y-auto py-4 px-3">
            ...
            <div class="mt-6">
                ...
                <div class="space-y-1">
                    ...
                    />
                    <NavItem
                        to="/siswa/ubah-password"
                        icon="lucide:lock-keyhole"
                        label="Ubah Password"
                        :active="$route.path.startsWith('/siswa/ubah-password')"
                        :minimized="isMinimized"
                    />
                </div>
            </div>
        ...
        <div class="border-t border-slate-100 p-3 space-y-2 shrink-0">
            ...
            >
                <div
                    class="w-9 h-9 rounded-lg bg-sky-500 flex items-center justify-center text-white font-semibold text-sm shrink-0 shadow-sm"
                >
                    {{ userInitials }}
                </div>
                <Transition name="fade">
                    <div v-show="!isMinimized" class="flex-1 min-w-0">
                        ...
                        <p class="text-xs text-slate-500">{{ userClass }}</p>
                    </div>
                </Transition>
                <Icon
                    v-show="!isMinimized"
...
// Inject profile from layout
const profile = inject<{
  userName: Ref<string>;
  userInitials: Ref<string>;
  userClass: Ref<string>;
}>('siswaProfile', {
  userName: ref('Siswa'),
  userInitials: ref('S'),
  userClass: ref(''),
...
const userName = computed(() => profile.userName.value || 'Siswa');
const userInitials = computed(() => profile.userInitials.value || 'S');
const userClass = computed(() => profile.userClass.value || '-');
                    ...
                    h("div", { class: "relative" }, [
                        h(resolveComponent("Icon"), {
                            ...
                            class: [
                                ...
                                props.active ? "" : "group-hover:scale-110",
                            ],
                        }),
                        props.showDot &&
                            h("span", {
